...
def read_blast(sequences,input_file):
	...
	for line in input_file:
		...
		hsp.gaps=int(line.split("\t")[9])
		hsp.score=float(line.split("\t")[11])
		hsp.identities=int(line.split("\t")[7])
		hsp.e_value=float(line.split("\t")[10])
...
def alter_names_of_unknown(seq_id,sequences,seq_id2interpro):
	...
	for interpro_result in sorted_interpro_results:
		if sequences[seq_id].name.lower()=="hypothetical protein":
			if interpro_result.name.lower().find("protein of unknown function")==-1 and interpro_result.name.lower().find("uncharacterised protein")==-1:
	...
	def __init__(self):
		if len(GO_graph.ec_to_go.keys())==0:
			...
			for line in obo_file:
				...
				if line[0:6]=="is_a: " or line[0:22]=="relationship: part_of ": # relationship: part_of was added because of methylation (see GO:0009008)
